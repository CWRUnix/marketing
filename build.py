...
for svg in svgs:
    ...
    for color in colors:
        ...
        fname = svg.parent / f"{cname}-{svg.name}"
        script += f'''
        ...
        if template.exists():
            ...
            for item in temp_vals:
                ...
                script += f'''
                    ...
                    inkscape -o "./out/{temp_fname.parent}/solid-{temp_fname.name}" --export-type=png --export-width=500 --export-background=$BG "./out/{temp_fname}"
                    inkscape -o "./out/{temp_fname.parent}/clear-{temp_fname.name}" --export-type=png --export-width=500 "./out/{temp_fname}"
                '''
        else:
            script += f'''
                inkscape -o "./out/{fname}" --export-type=svg --export-plain-svg "./{fname}"
                inkscape -o "./out/{fname.parent}/solid-{fname.name}" --export-type=png --export-width=500 --export-background=$BG "./out/{fname}"
                inkscape -o "./out/{fname.parent}/clear-{fname.name}" --export-type=png --export-width=500 "./out/{fname}"
            '''
